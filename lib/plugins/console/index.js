...
function registerInternalConsolePlugin(ctx){
    ...
    console.register('init', 'Create a new sorest folder.', initOptions, require('./init'));

    console.register('version', 'Display version information.', require('./version'));

    var startOptions = {
        usage: '[production] <filename>',
        arguments: [
            {name: 'production', desc: 'running in production mode, otherwise development mode by default. (optional)'}
        ],
        options: [
            {name: '-f, --file', desc: 'Specify the configuration file. (required)'}
        ]
    };
    console.register('start', 'start a sorest server', startOptions, require("./start"));
}